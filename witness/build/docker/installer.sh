...
run_node()
{
    ...
    echo "* Run node"
    ${SUDO} docker run --log-driver syslog -v ${_WORK_DIR}:/var/lib/playchain ${_ARGS} -d \
                ${_ENVS} \
...
main()
{
    ...
    echo ">> CONFIG_WORK_DIR is $CONFIG_WORK_DIR"
    ...
    if [[ -z "$CONFIG_WORK_DIR" ]]; then
        _WORK_DIR=${USER_HOME}/.local/share/${DOCKER_IMAGE_NAME}
    else
        _WORK_DIR=${CONFIG_WORK_DIR}
    fi
    ...
    if [[ $CURRENT_CHECK_SUM != $CHECK_SUM || \
     ! -e ${_WORK_DIR}/playchain.config.ini ]]; then
        ...
        ${SUDO} docker load -i ${DOCKER_IMAGE_NAME}
        ${UNSUDO} mkdir -p ${_WORK_DIR}
        local CONF_RESULT=0
        if [[ -z ${PLAYCHAIN_NODE_CONFIG_NOT_INTERACTIVE} ]]; then
            ...
            ${SUDO} docker run -v ${_WORK_DIR}:/var/lib/playchain -it \
                ${_ENVS} \
